...
public class PackageMojo extends AbstractMojo {
    ...
    public void execute() {
        try {
            ...
            getLog().info("prepare work dir...");
            File workdir = new File(workingDirectory.getPath() + "/" + DOCKER_DIR);
            if (!workdir.exists()) {
                workdir.mkdirs();
            }
            String jarName = project.getArtifactId() + "-" + project.getVersion() + ".jar";
            String[] args = new String[]{"java", "-Djarmode=layertools", "-jar", jarName, "extract", "--destination", workdir.getPath()};
            getLog().info("execute cmd: " + String.join(" ", args));
            ...
            getLog().info("Write package file to dir: " + workingDirectory.getPath());
    ...
    private void createShFile(String workingDirectoryPath) throws IOException {
        getLog().info("begin create sh file.");
        File shFile = new File(workingDirectoryPath + "/" + DOCKER_DIR, "run.sh");
        if (!shFile.exists()) {
            getLog().info("sh file not exists, create it, file path: " + shFile.getPath());
            if (!shFile.createNewFile()) {
                getLog().error("create sh file failed.");
                return;
            }
            getLog().info("sh file created.");
        }
        if (shFile.canWrite()) {
